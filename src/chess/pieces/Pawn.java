	...
	public boolean[][] possibleMoves() {
		...
		if (getColor() == Color.WHITE) {
			...
			if (getBoard().positionExists(p) && !getBoard().thereIsAPiece(p) && getBoard().positionExists(p2)
					&& !getBoard().thereIsAPiece(p2) && getMoveCount() == 0) { // regra de movimentação
			...
			if (getBoard().positionExists(p) && isThereOpponentPiece(p)) { // verificando oponente na diagonal
				...
				if (position.getRow() == 3) {
					...
					if (getBoard().positionExists(left) && isThereOpponentPiece(left)
							&& getBoard().piece(left) == chessMatch.getEnPassantVulnerable()) { // verificação da
																								// pósição
						mat[left.getRow() - 1][left.getColumn()] = true;
					}
					Position right = new Position(position.getRow(), position.getColumn() + 1);
					if (getBoard().positionExists(right) && isThereOpponentPiece(right)
							&& getBoard().piece(right) == chessMatch.getEnPassantVulnerable()) { // verificação da
																									// pósição
			...
			p.setValues(position.getRow() + 2, position.getColumn());
			Position p2 = new Position(position.getRow() + 1, position.getColumn());// alterou para + pois as peças
																					// pretas do peão estavam andando
																					// apenas 1 linha.
			if (getBoard().positionExists(p) && !getBoard().thereIsAPiece(p) && getBoard().positionExists(p2)
					&& !getBoard().thereIsAPiece(p2) && getMoveCount() == 0) { // regra de movimentação
				mat[p.getRow()][p.getColumn()] = true; // verificando as duas casas em frente
